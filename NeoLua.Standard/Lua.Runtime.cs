			...
			if (state == 0) // a integer value
      {
		...
		private static object ThrowFormatExpression(bool lThrowException, string sNumber, int iBase)
		{
			if (lThrowException)
			{
				...
				throw new FormatException(String.Format(Resources.rsFormatError, sNumber, sType));
			}
			else
				return null;
		...
		internal static Delegate RtConvertDelegate(Type toType, Delegate dlg)
		{
			...
			{
				throw new LuaRuntimeException(String.Format(Resources.rsBindConversionNotDefined, dlg.GetType().Name, toType.Name), e);
			}
		...
		public static int RtLength(object v)
		{
			...
			else
			{
				...
				throw new LuaRuntimeException(String.Format(Resources.rsNoLengthOperator, lt.AliasName ?? lt.Name), null);
			}
		} // func RtLength
