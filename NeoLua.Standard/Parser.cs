	...
	internal static partial class Parser
	{
		...
		private sealed class ArgumentsList
		{
			...
			public void AddPositionalArgument(Token position, Expression expr)
			{
				...
				if (names.Count > 0)
					throw ParseError(position, Resources.rsParseInvalidArgList);
		...
		private class PrefixMemberInfo
		{
			...
			public Expression GenerateSet(Scope scope, Expression exprToSet)
			{
				...
				else if (Instance != null && Member == null && Indices == null && Arguments == null && Instance is ParameterExpression)
				{
					...
					throw ParseError(Position, Resources.rsParseExpressionNotAssignable);
			...
			public Expression GenerateGet(Scope scope, InvokeResult result)
			{
				...
				else if (Instance != null && Indices == null && (Arguments != null || MethodMember))
				{
					...
					throw ParseError(Position, Resources.rsParseExpressionNoResult);
      ...
      if ((globalScope.EmitDebug & LuaDebugLevel.RegisterMethods) == LuaDebugLevel.RegisterMethods)
			...
			if (code.Current.Typ != LuaToken.Eof)
				throw ParseError(code.Current, Resources.rsParseEof);
		...
		private static bool ParseStatement(Scope scope, LuaLexer code)
		{
			switch (code.Current.Typ)
			{
				...
				case LuaToken.InvalidString:
					throw ParseError(code.Current, Resources.rsParseInvalidString);
				case LuaToken.InvalidComment:
					throw ParseError(code.Current, Resources.rsParseInvalidComment);
				case LuaToken.InvalidChar:
					throw ParseError(code.Current, Resources.rsParseInvalidChar);
			...
			while (true)
			{
				if (isLocal) // parse local variables
				{
					...
					if (exprVar == null)
					{
						...
						throw ParseError(tVar, Resources.rsParseTypeRedef);
			...
			else if (!isLocal)
			{
				for (int i = 0; i < prefixes.Count; i++)
				{
					if (prefixes[i].Arguments == null) // do not execute getMember
						throw ParseError(prefixes[i].Position, Resources.rsParseAssignmentExpected);
		...
		private static Expression ParseElseStatement(Scope scope, LuaLexer code)
		{
			...
			else if (code.Current.Typ == LuaToken.KwEnd)
			{
				...
				throw ParseError(code.Current, Resources.rsParseUnexpectedTokenElse);
		...
		private static void ParseConst(Scope scope, LuaLexer code)
		{
			...
			else
			{
				...
				if (exprConst.Type == typeof(object) || exprConst.Type == typeof(LuaResult)) // dynamic calls, no constant possible
					throw ParseError(tVarName, Resources.rsConstExpressionNeeded);
				else
					try
					{
					...
					{
						throw ParseError(tVarName, String.Format(Resources.rsConstExpressionEvalError, e.Message));
					}
			}
		} // func ParseConst
		...
		private static PrefixMemberInfo ParsePrefix(Scope scope, LuaLexer code)
		{
			...
			switch (tStart.Typ)
			{
				...
				case LuaToken.KwForEach:
					...
					else
					{
						...
						if (t.Typ == LuaToken.DotDotDot && p == null)
							throw ParseError(t, Resources.rsParseNoArgList);
						code.Next();
				...
				default:
					throw ParseError(code.Current, Resources.rsParseUnexpectedTokenPrefix);
			}
		...
		private static Expression ParseNumber(Lua runtime, Token t)
		{
			...
			else
			{
				...
				if (v != null)
					...
					throw ParseError(t, String.Format(Resources.rsParseConvertNumberError, sNumber));
			}
		} // func ParseNumber
				...
				if (code.Current.Typ == LuaToken.BracketSquareOpen)
				{
					...
					if (genericeTypes.Count == 0) // create a array at the end
					{
						if (currentType.Type == null)
							throw ParseError(code.Current, String.Format(Resources.rsParseUnknownType, currentType.FullName));
					...
					if (code.Current.Typ == LuaToken.BracketOpen)
					{
						...
						if (code.Current.Typ == LuaToken.Colon)
						{
							...
							if (!typeof(Exception).GetTypeInfo().IsAssignableFrom(exceptionType.GetTypeInfo()))
								throw ParseError(code.Current, String.Format(Resources.rsParseCatchVarTypeMustAssignableToException, exceptionType.FullName));
						}
						else
		...
		private static void ParseForEachLoop(Scope scope, LuaLexer code)
		{
			...
			scope.AddExpression(
				...
				Expression.IfThen(Expression.Equal(varEnumerable, Expression.Constant(null, typeof(object))), Lua.ThrowExpression(Resources.rsExpressionNotEnumerable)),
		...
		private static Token FetchToken(LuaToken typ, LuaLexer code, bool isOptional = false)
		{
			if (code.Current.Typ == typ)
			{
				...
				throw ParseError(code.Current, String.Format(Resources.rsParseUnexpectedToken, LuaLexer.GetTokenName(code.Current.Typ), LuaLexer.GetTokenName(typ)));
		} // proc FetchToken
