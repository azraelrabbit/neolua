...
namespace Neo.IronLua
{
	...
	public class LuaTable : IDynamicMetaObjectProvider, INotifyPropertyChanged, IDictionary<object, object>
	{
		...
		private sealed class LuaTableMetaObject : DynamicMetaObject
		{
			...
			public override DynamicMetaObject BindSetIndex(SetIndexBinder binder, DynamicMetaObject[] indexes, DynamicMetaObject value)
			{
				...
				if (indexes.Length == 1)
				{
					...
					if (arg.Value == null)
					{
						expr = Lua.ThrowExpression(Resources.rsTableKeyNotNullable, typeof(object));
			...
			public override DynamicMetaObject BindInvokeMember(InvokeMemberBinder binder, DynamicMetaObject[] args)
			{
				...
				var expr = Expression.Block(
					...
					Expression.Switch(
						...
						Expression.SwitchCase(Lua.ThrowExpression(String.Format(Resources.rsMemberNotResolved, "table", binder.Name), typeof(object)), Expression.Constant(CallMethod.Nil)),
		...
		private sealed class MemberImplementation : IDictionary<string, object>
		{
			...
			{
				if (key == null)
					throw new ArgumentNullException(Resources.rsTableKeyNotNullable);
					...
					else // create overloaded member
					{
						...
						for (var j = 1; j < methods.Length; j++)
						{
							if (methods[0].IsStatic != methods[j].IsStatic)
								throw new ArgumentException(String.Format(Resources.rsMethodStaticMix, methods[0]));
		...
		private int SetMemberValueIntern(string memberName, object value, bool ignoreCase, bool rawSet, bool add, bool markAsMethod)
		{
			...
			else if (entryIndex >= 0) // key will be setted
			{
				// only add is allowed
				if (add)
					throw new ArgumentException(String.Format(Resources.rsTableAddDuplicate, memberName));
		...
		public Delegate DefineMethod(string methodName, Delegate method, bool ignoreCase = false)
		{
			...
			if (!typeFirstParameter.GetTypeInfo().IsAssignableFrom(typeof(LuaTable).GetTypeInfo()))
				throw new ArgumentException(String.Format(Resources.rsTableMethodExpected, methodName));
		...
		public LuaResult CallMemberDirect(string memberName, object[] args, bool ignoreCase = false, bool rawGet = false, bool throwExceptions = true, bool ignoreNilFunction = false)
		{
				...
				switch (GetCallMember(memberName, ignoreCase, rawGet, out var method))
				{
					case CallMethod.Nil:
						if (throwExceptions && !ignoreNilFunction)
							throw new ArgumentNullException(String.Format(Resources.rsMemberNotResolved, "table", memberName));
						else
			...
			{
				if (throwExceptions)
					throw new TargetInvocationException(String.Format(Resources.rsTableCallMemberFailed, memberName), e.InnerException);
				return LuaResult.Empty;
			}
		} // func CallMemberDirect

		public object RtInvokeSite(object target, params object[] args)
		{
		...
		private bool TryInvokeMetaTableOperator<TRETURN>(string key, bool raise, out TRETURN r, params object[] args)
		{
			if (metaTable != null)
			{
				...
				if (o != null)
				{
					...
					if (raise)
						throw new LuaRuntimeException(String.Format(Resources.rsTableOperatorIncompatible, key, "function"), 0, true);
				}
			}
			if (raise)
				throw new LuaRuntimeException(String.Format(Resources.rsTableOperatorNotFound, key), 0, true);
		...
		{
			if (ContainsKey(key))
				throw new ArgumentException(String.Format(Resources.rsTableAddDuplicate, key));
		...
		void ICollection<KeyValuePair<object, object>>.Add(KeyValuePair<object, object> item)
		{
			if (ContainsKey(item.Key))
				throw new ArgumentException(String.Format(Resources.rsTableAddDuplicate, item.Key));
