...
namespace Neo.IronLua
{
	...
	public sealed class LuaType : IDynamicMetaObjectProvider
	{
		...
		private class LuaTypeMetaObject : DynamicMetaObject
		{
				...
				if (type != null) // there is a valid type
				{
					...
					else if (r == LuaTryGetMemberReturn.NotReadable)
					{
						return new DynamicMetaObject(
							Lua.ThrowExpression(
								String.Format(Resources.rsMemberNotReadable, type.Name, binder.Name),
								binder.ReturnType
							),
							restrictions
						);
			...
			public override DynamicMetaObject BindSetMember(SetMemberBinder binder, DynamicMetaObject value)
			{
					...
					else
					{
						return new DynamicMetaObject(
							Lua.ThrowExpression(String.Format(
								r == LuaTrySetMemberReturn.NotWritable ? Resources.rsMemberNotWritable : Resources.rsMemberNotResolved,
				...
				else
				{
					var expr = Expression.Condition(
						GetUpdateCondition(),
						binder.GetUpdateExpression(binder.ReturnType),
						Lua.ThrowExpression(String.Format(Resources.rsMemberNotWritable, "LuaType", binder.Name), binder.ReturnType)
					);
					return new DynamicMetaObject(expr, restrictions);
				}
			} // proc BindSetMember
			...
			{
				return new DynamicMetaObject(
					Lua.ThrowExpression(Resources.rsIndexNotFound, binder.ReturnType),
			...
			public override DynamicMetaObject BindInvokeMember(InvokeMemberBinder binder, DynamicMetaObject[] args)
			{
				...
				else if (type == null) // type not resolved
				{
					...
					var fallback = binder.FallbackInvokeMember(this, args, new DynamicMetaObject(Lua.ThrowExpression(String.Format(Resources.rsParseUnknownType, luaType.FullName), binder.ReturnType), restrictions));
					...
					try
					{
						...
						else
						{
							if (String.Compare(binder.Name, "ctor", stringComparison) == 0)
								...
								return binder.FallbackInvokeMember(this, args, new DynamicMetaObject(Lua.ThrowExpression(String.Format(Resources.rsMemberNotResolved, luaType.FullName, binder.Name), binder.ReturnType), restrictions));
						}
					}
					catch (LuaEmitException e)
					{
			...
			public override DynamicMetaObject BindInvoke(InvokeBinder binder, DynamicMetaObject[] args)
			{
				...
				if (type == null) // type not resolved
				{
					...
					var fallback = binder.FallbackInvoke(this, args, new DynamicMetaObject(Lua.ThrowExpression(Resources.rsNullReference, binder.ReturnType), restrictions));
			...
			private DynamicMetaObject BindNewObject(LuaType luaType, CallInfo callInfo, DynamicMetaObject[] args, Type returnType)
			{
				...
				if (constructorInfo == null && !isValueType)
					return new DynamicMetaObject(Lua.ThrowExpression(String.Format(Resources.rsMemberNotResolved, luaType.FullName, "ctor"), returnType), restrictions);
		...
		private void RegisterExtension(MethodInfo mi, bool checkFirstParameter)
		{
			...
			if (checkFirstParameter && parameterInfo.Length == 0)
				throw new ArgumentException(String.Format(Resources.rsTypeExtentionInvalidMethod, mi.DeclaringType.Name, mi.Name));
		...
		private Type MakeGenericClrType(LuaType[] genericArguments, bool throwException)
		{
			...
			{
				if (throwException)
					throw new ArgumentException(String.Format(Resources.rsParseUnknownType, genericTypeDefinitionString));
				else
					return null;
			}
			...
			for (var i = 0; i < genericClrArguments.Length; i++)
			{
				...
				{
					if (throwException)
						throw new ArgumentException(String.Format(Resources.rsParseUnknownType, "<" + i.ToString() + ">" + genericArguments[i].FullName));
					else
						return null;
				}
		...
		public LuaType MakeArrayLuaType(int rank = 1, bool throwException = true)
		{
			...
			{
				if (throwException)
					throw new ArgumentException(String.Format(Resources.rsParseUnknownType, FullName));
				else
					return null;
			}
			else
		...
		private static Exception TypeParseException(int offset, string fullName, string expected)
			=> new FormatException(String.Format(Resources.rsTypeParseError, fullName, offset >= fullName.Length ? "eof" : fullName.Substring(offset, 1), offset, expected));
		...
		public static LuaType GetType(Type type)
		{
			...
			if (fullName == null)
			{
				if (type.IsConstructedGenericType)
				{
					...
					throw new ArgumentNullException(String.Format(Resources.rsTypeInvalidType, type.Name));
			}
		...
		public static void RegisterTypeAlias(string aliasName, Type type)
		{
			if (aliasName.IndexOfAny(new char[] { '.', '+', ' ', '[', ']', ',' }) >= 0)
				throw new ArgumentException(String.Format(Resources.rsTypeAliasInvalidName, aliasName));
		...
		public static void RegisterTypeExtension(Type type)
		{
			...
			if (typeInfo.IsSealed && typeInfo.IsAbstract)
			{
				...
				throw new ArgumentException(String.Format(Resources.rsTypeExtentionInvalidType, type.Name));
	...
	public sealed class LuaOverloadedMethod : ILuaMethod, IDynamicMetaObjectProvider, IEnumerable<Delegate>
	{
		...
		private class LuaOverloadedMethodMetaObject : DynamicMetaObject
		{
			...
			public override DynamicMetaObject BindGetIndex(GetIndexBinder binder, DynamicMetaObject[] indexes)
			{
				...
				{
					return new DynamicMetaObject(
						Lua.ThrowExpression(String.Format(Resources.rsClrGenericTypeExpected)),
				...
				if (mi == null)
				{
					var fallback = binder.FallbackInvoke(this, args,
						new DynamicMetaObject(
							Lua.ThrowExpression(String.Format(Resources.rsMemberNotResolved, val.Type, val.Name)),
