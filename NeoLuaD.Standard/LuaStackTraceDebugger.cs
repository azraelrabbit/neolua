...
namespace Neo.IronLua
{
	...
	public sealed class LuaStackTraceDebugger : ILuaDebug
	{
		...
		private class LuaStackTraceChunk : LuaChunk
		{
		    ...
		    public override ILuaDebugInfo GetDebugInfo(MethodBase method, int ilOffset)
			{
		...
		LuaChunk ILuaDebug.CreateChunk(Lua lua, LambdaExpression expr)
		{
			lock (this)
			{
				...
				if (assembly == null)
				{
				    assembly = AssemblyBuilder.DefineDynamicAssembly(GetLuaDynamicName(), AssemblyBuilderAccess.RunAndCollect);
					//assembly = AppDomain.CurrentDomain.DefineDynamicAssembly(GetLuaDynamicName(), AssemblyBuilderAccess.RunAndCollect);
					module = assembly.DefineDynamicModule("lua");
				}
				...
				//var typeFinished = type.CreateTypeInfo().CreateType();

				// Initialize fields, create the static callsite's
				//reduce.InitMethods(typeFinished);

				// return chunk
				return chunk.InitChunk(expr.Compile(collectedDebugInfo));
			}
