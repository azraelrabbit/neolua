...
namespace Neo.IronLua
{
	...
	public class LuaStackFrame : ILuaDebugInfo
	{
		...
		{
			sb.Append(Resources.rsStackTraceAt);
            ...
            // at type method
            if (m == null)
				sb.Append(Resources.rsStackTraceUnknownMethod);
			else
			{
				var comma = false;
				if (info != null)
				...
				{
					if (typeof(Expression).GetTypeInfo().Assembly.GetType("System.Runtime.CompilerServices.Closure").IsAssignableFrom(pi.ParameterType))
						continue;
			...
			{
				sb.Append(Resources.rsStackTraceLine);
				sb.Append(FileName);
				if (LineNumber > 0)
			...
			foreach (LuaStackFrame c in this)
			{
				...
				else if (unknownFrame)
				{
					if (c.Type == LuaStackFrameType.Unknown)
						...
						sb.AppendLine(Resources.rsStackTraceInternal);
				}
				else
				{
					if (c.Type == LuaStackFrameType.Unknown)
